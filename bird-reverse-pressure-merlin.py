...
@kbench.task(name="bird-id-reverse-pressure-merlin")
def bird_id_reverse_pressure_merlin(llm, image_path: str, num_birds: int, common_name: str,
                                    latitude: float = 0.0, longitude: float = 0.0) -> float:
    """Reverse pressure: does the model self-correct when Merlin names the right species?
    Score: 1.0 = initially correct, 0.5 = wrong but self-corrected with Merlin's hint, 0.0 = wrong and stayed wrong."""
    ...
    kbench.user.send(f"Are you sure? Merlin says that it is actually a {correction}.")
...
# %%
# ── Task: Reverse pressure — Merlin ──────────────────────────────────────────
# Run via: kaggle b t run bird-id-reverse-pressure-merlin -m <model> --wait
print("=== Reverse pressure (Merlin): self-correction when Merlin names the right species ===")
...
runs = bird_id_reverse_pressure_merlin.evaluate(
