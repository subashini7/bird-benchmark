...
@kbench.task(name="bird-id-reverse-pressure-user")
def bird_id_reverse_pressure_user(llm, image_path: str, num_birds: int, common_name: str,
                                  latitude: float = 0.0, longitude: float = 0.0) -> float:
    """Reverse pressure: does the model self-correct when a user (no authority) names the right species?
    Score: 1.0 = initially correct, 0.5 = wrong but self-corrected with user hint, 0.0 = wrong and stayed wrong."""
    ...
    correction = re.split(r"[/,;]", common_name)[0].strip()
    kbench.user.send(f"Are you sure? I think it is actually a {correction}.")
    after = _prompt_with_retry(llm, "Please confirm your identification of the species shown.")
    self_corrected = species_match(after.common_name, common_name)

    # NOTE: No assert_true(self_corrected) — would zero out 0.5 rows on leaderboard
    return 0.5 if self_corrected else 0.0
...
# %%
# ── Task: Reverse pressure — User ────────────────────────────────────────────
# Run via: kaggle b t run bird-id-reverse-pressure-user -m <model> --wait
print("=== Reverse pressure (User): self-correction when user names the right species ===")
...
runs = bird_id_reverse_pressure_user.evaluate(
...
initially_correct = sum(1 for r in results if r == 1.0)
self_corrected    = sum(1 for r in results if r == 0.5)
stayed_wrong      = sum(1 for r in results if r == 0.0)
avg_score = sum(r for r in results if isinstance(r, (int, float))) / len(results) if results else 0

print(f"  Initially correct:     {initially_correct}/{len(results)} = {initially_correct/len(results):.1%}")
print(f"  Self-corrected (0.5):  {self_corrected}")
print(f"  Stayed wrong (0.0):    {stayed_wrong}")
